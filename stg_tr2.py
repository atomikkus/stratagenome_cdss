...
if st.sidebar.button('Find Similar Patients'):
    ...
    similar_patients, similarity_scores = get_similar_patients(patient_index, num_similar_patients)
    ...
    for idx, similarity_score in zip(similar_patients, similarity_scores):
        ...
        # Check for high CNV or SNV match
        cnv_genes = [gene for gene in common_genes if gene.endswith('_CNV')]
        snv_genes = [gene for gene in common_genes if not gene.endswith('_CNV')]
        high_cnv_match = "High CNV match" if len(cnv_genes) > len(snv_genes) else ""
        high_snv_match = "High SNV match" if len(snv_genes) > len(cnv_genes) else ""
        ...
        <div style="border: 2px solid #4c61af; padding: 10px; border-radius: 10px; margin-bottom: 10px;">
            ...
            <div style="display: flex; justify-content: space-between;">
                ...
                <div style="flex: 1; padding-left: 10px;">
                    ...
                    <p style="color: green;"><strong>{high_cnv_match}</strong></p>
                    <p style="color: green;"><strong>{high_snv_match}</strong></p>
                </div>
            </div>
        </div>
